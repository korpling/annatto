...
use crate::{Module, error::AnnattoError, util::insert_corpus_nodes_from_path};
...
fn import_workbook(update: &mut GraphUpdate, path: &Path, column_map: &BTreeMap<String, Vec<String>>) -> Result<(), Box<dyn std::error::Error>> {    
    ...
    let rownums_by_col0i = {
        ...
        for col_0i in name_to_col_0index.values() {
            m.insert(*col_0i, (2 .. sheet.get_highest_row() + 2).collect::<BTreeSet<u32>>());
        }
    ...
    if is_multi_tok {
        ...
        base_tokens.iter().tuple_windows().try_for_each(|(first, second)| {
    ...
    for (tok_name, anno_names) in column_map.into_iter() {
        ...
        if let Some(col_0i) = index_opt {
            ...
            for (start_row, end_row_excl) in row_nums.into_iter().tuple_windows() {
                let cell = sheet.get_cell_by_column_and_row(&(col_0i + 1), start_row).unwrap();
                ...
                if is_multi_tok {
                    let overlapped_tokens: &[String] = &base_tokens[*start_row as usize - 2..*end_row_excl as usize - 2];
                    let span_name = format!("{}#span{}", &doc_path, next_span_num);
                    next_span_num += 1;
                    update.add_event(UpdateEvent::AddNode { node_name: span_name.to_string(), node_type: "node".to_string() })?;
                    update.add_event(UpdateEvent::AddNodeLabel { node_name: span_name.to_string(), anno_ns: ANNIS_NS.to_string(), anno_name: "tok".to_string(), anno_value: value.to_string() })?;
                    update.add_event(UpdateEvent::AddNodeLabel { node_name: span_name.to_string(), anno_ns: "".to_string(), anno_name: tok_name.to_string(), anno_value: value.to_string() })?;
                    ...
                    next_tok_num += 1;
                    update.add_event(UpdateEvent::AddNode { node_name: node_name.to_string(), node_type: "node".to_string() })?;
                    update.add_event(UpdateEvent::AddNodeLabel { node_name: node_name.to_string(), anno_ns: ANNIS_NS.to_string(), anno_name: "tok".to_string(), anno_value: value.to_string() })?;
                    update.add_event(UpdateEvent::AddNodeLabel { node_name: node_name.to_string(), anno_ns: "".to_string(), anno_name: tok_name.to_string(), anno_value: value.to_string() })?;
                    for row_num in *start_row .. *end_row_excl {
        ...
        token_map.iter().sorted().tuple_windows().try_for_each(|((_, first_id), (_, second_id))| {
            update.add_event(UpdateEvent::AddEdge { 
        ...
        for qname in anno_names {
            ...
            if let Some(col_0i) = index_opt {
                ...
                for (start_row, end_row_excl) in row_nums.into_iter().tuple_windows() {
                    ...
                    next_span_num += 1;
                    update.add_event(UpdateEvent::AddNode { node_name: span_name.to_string(), node_type: "node".to_string() })?;
                    update.add_event(UpdateEvent::AddNodeLabel { node_name: span_name.to_string(), anno_ns: anno_ns.to_string(), anno_name: anno_name.to_string(), anno_value: value.to_string() })?;
                    for covered_token in overlapped_tokens {
                        update.add_event(UpdateEvent::AddEdge { source_node: span_name.to_string(), target_node: covered_token.to_string(), layer: ANNIS_NS.to_string(), component_type: AnnotationComponentType::Coverage.to_string(), component_name: "".to_string() })?;
                    }
                }
            } else {
                // TODO warning
...
fn get_column_map(property_val: &String) -> Result<BTreeMap<String, Vec<String>>, Box<dyn std::error::Error>> {  // TODO produce some errors
    let mut column_map = BTreeMap::new();
    for group in property_val.split(";") {
        let (key, names) = match group.trim().split_once("=") {
            None => {
                let err = AnnattoError::InvalidPropertyValue { property: PROP_COLUMN_MAP.to_string(), value: property_val.to_string() };
                return Err(Box::new(err));
            },
            Some((k, v)) => {
                let anno_names = v
...
mod tests {
    ...
    use graphannis::{corpusstorage::{QueryLanguage, SearchQuery}, CorpusStorage, AnnotationGraph};
