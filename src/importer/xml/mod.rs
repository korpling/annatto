        ...
        loop {
            ...
            match xml_event {
                xml::reader::XmlEvent::StartElement {
                    ...
                    let parent_node_id = match node_stack.last() {
                        None => {
                            return Err(AnnattoError::Import {
                                reason: format!(
                                    "{} cannot be mapped. Node stack is empty.",
                    ...
                    let node_id = format!("{}#n{node_count}", doc_node_id.to_string());
                    ...
                    update.add_event(UpdateEvent::AddEdge {
                        ...
                        target_node: parent_node_id.to_string(),
                    ...
                    if node_stack.len() > 1 {
                        update.add_event(UpdateEvent::AddEdge {
                            source_node: parent_node_id.to_string(),
                    ...
                    if self.default_ordering == lookup {
                        ...
                        ordering_ends_at.insert("".to_string(), node_id.to_string());
                        update.add_event(UpdateEvent::AddNodeLabel {
                            node_name: node_id.to_string(),
                ...
                xml::reader::XmlEvent::EndElement { name } => {
                    if let Some(node_id) = node_stack.pop() {
                        ...
                        let lookup = name.to_string();
                        if self.default_ordering == lookup || self.named_orderings.contains(&lookup)
                        {
                            update.add_event(UpdateEvent::AddNodeLabel {
                                node_name: node_id.to_string(),
                                anno_ns: ANNIS_NS.to_string(),
                ...
                xml::reader::XmlEvent::Characters(chars) => char_buffer.push_str(chars.as_str()),
