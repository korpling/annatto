...
mod tests {
    ...
    fn exb_conversion() {
        ...
        .unwrap();
        let exporter = JavaExporter::new(
            "org.corpus_tools.peppermodules.exmaralda.EXMARaLDAExporter",
            "org.corpus_tools.peppermodules.exmaralda.EXMARaLDAExporterProperties",
            "EXMARaLDAExporter",
            true,
        )
        .unwrap();
        let properties: BTreeMap<String, String> = BTreeMap::new();
        let input = PathBuf::from("test-corpora/exb/rootCorpus");
        let mut updates = importer
            ...
            .unwrap();
        // Create an annotation graph from these updates
        let mut g = AnnotationGraph::new(false).unwrap();
        g.apply_update(&mut updates, |_| {}).unwrap();

        // Export the corpus to a temporary directory
        let output = tempfile::tempdir().unwrap();
        exporter
            .export_corpus(&g, &properties, output.path(), None)
            .unwrap();

        // Test the output files.
        // Since we use test files generated by the exporter,
        // we expect the strings to be the identical (and not only the XML structure).
        for f in &[
            "subCorpus1/doc1.exb, subCorpus1/doc2.exb",
            "subCorpus2/doc1.exb",
            "subCorpus2/doc2.exb",
        ] {
            let f = PathBuf::from(f);
            let input_path: PathBuf = [&input, &f].iter().collect();
            let output_path: PathBuf = [output.path(), &f].iter().collect();
            let mut input_file = File::open(&input_path).unwrap();
            let mut output_file = File::open(&output_path).unwrap();

            assert_eq!(true, diff_files(&mut input_file, &mut output_file));
        }
    }
}
