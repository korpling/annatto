...
use crate::{workflow::StatusSender, Manipulator, Module};
...
impl Manipulator for CheckingMergeFinalizer {
    fn manipulate_corpus(
        ...
        if let Some(sender) = &tx {
            sender.send(crate::workflow::StatusMessage::Info(String::from(
                "Starting merge check",
            )))?;
        }
        let on_error = OnErrorValues::try_from(properties.get(PROP_ON_ERROR))?;
        let qnames = properties
            .get(PROP_CHECK_NAMES)
            .unwrap()
            .split(";")
            .collect::<Vec<&str>>();
        let search_name_tuples = qnames
            .iter()
            .map(|qn| split_qname(qn))
            .collect::<Vec<(&str, &str)>>();
        ...
        for result in node_annos.exact_anno_search(Some("annis"), "tok", ValueSearch::Any) {
            ...
            let anno_values = search_name_tuples.iter().map(|tpl| {
                node_annos.get_value_for_item(
                    &node_id,
                    &AnnoKey {
                        ns: smartstring::alias::String::from(tpl.0),
                        name: smartstring::alias::String::from(tpl.1),
                    },
                )
            });
            ...
            for name_tuple in &search_name_tuples {
                ...
                if !name.eq(split_keep_name.1) || !ns.eq(split_keep_name.0) {
                    updates.add_event(UpdateEvent::DeleteNodeLabel {
                        ...
                        anno_ns: String::from(ns),
                        anno_name: String::from(name),
                    })?;
                }
            }
        }
        if docs_with_errors.len() > 0 {
            ...
            if let Some(sender) = &tx {
                sender.send(crate::workflow::StatusMessage::Warning(format!(
                    "Documents with ill-merged tokens:\n{}",
                    msg
                )))?;
            }
            match on_error {
                OnErrorValues::Fail => {
                    return Err(Box::new(AnnattoError::Manipulator {
                        reason: String::from("Mismatching tokens in some documents."),
                        manipulator: String::from(self.module_name()),
                    }))
                }
                OnErrorValues::Drop => {
                    for doc_node_id in docs_with_errors {
                        updates.add_event(UpdateEvent::DeleteNode {
                            node_name: doc_node_id,
                        })?;
                    }
                }
                _ => {}
