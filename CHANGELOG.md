...
- graph op `check`: Allow to configure exhaustive annotation name test, i. e., prescribe which annotations are allowed to exist and have to exist, with scopes "edge", "node", and "corpus".
...
- import `xlsx`: via `evaluate` formula evaluation can be activated. By default, formulas are imported as formula text values, not as the result of their evaluation.
...
- workflows can now be enriched with an initial `init` table to initialize the annotation graph as non-empty from an existing database (only in-memory)
- when saving a graph the graph statistics are calculated to have them ready for loading again
- configuring annotation components can now be done by providing `type` instead of `ctype`, but `ctype` is still valid
...
- add `git` import to incorporate git metadata
...
- import `webanno`: create part of-edges for tokens
